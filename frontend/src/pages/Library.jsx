...
const Library = () => {
      ...
      <div className="space-y-6 animate-fade-in">
        <header className="relative overflow-hidden rounded-3xl border border-white/[0.08] bg-gradient-to-br from-amber-500/20 via-amber-400/5 to-transparent p-6 shadow-xl sm:p-8">
          ...
          <div className="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="text-body mt-1.5 text-white/50">
                {validLikedSongs.length} {validLikedSongs.length === 1 ? "song" : "songs"}
              </p>
            </div>
          ...
          {validLikedSongs.length > 0 && (
            <button
              type="button"
              onClick={() => playSong(validLikedSongs[0], validLikedSongs, 0)}
            ...
            >
              <IoPlay className="text-base" /> Play All
            </button>
          )}
        </header>
          ...
          <div className="overflow-hidden rounded-2xl border border-white/[0.07] bg-[#101011]">
            <div className="divide-y divide-white/[0.03]">
              {validLikedSongs.map((song, i) => (
                <SongRow key={song._id || song.id || i} song={song} queue={validLikedSongs} index={i} showIndex />
              ))}
            </div>
          </div>
        )}
      ...
      <div className="space-y-6 animate-fade-in">
          ...
          <div className="relative z-10 flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="text-body mt-1 text-white/50">
                {validRecentlyPlayed.length} {validRecentlyPlayed.length === 1 ? "track recorded" : "tracks recorded in your playback history"}
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              {validRecentlyPlayed.length > 0 && (
                <>
                  <button
                    type="button"
                    onClick={() => playSong(validRecentlyPlayed[0], validRecentlyPlayed, 0)}
                  ...
                  <button
                    ...
                    onClick={() => {
                      setShuffle(true);
                      const randIdx = Math.floor(Math.random() * validRecentlyPlayed.length);
                      playSong(validRecentlyPlayed[randIdx], validRecentlyPlayed, randIdx);
                    }}
        ...
        {validRecentlyPlayed.length > 4 && (
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="text-xs text-white/40">{filteredRecentlyPlayed.length} of {validRecentlyPlayed.length} tracks</span>
          </div>
        )}
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {(tab === "all" || tab === "recent") && validRecentlyPlayed.length > 0 && (
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
          ...
          <div className="overflow-hidden rounded-2xl border border-white/[0.07] bg-[#101011]">
            <div className="divide-y divide-white/[0.03]">
              {validRecentlyPlayed.slice(0, tab === "recent" ? undefined : 6).map((song, i) => (
                <div
                ...
                >
                  <div className="min-w-0 flex-1">
                    <SongRow song={song} queue={validRecentlyPlayed} index={i} showIndex />
                  </div>
                  <button
                    type="button"
                    aria-label="Remove from recently played"
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            {validLikedSongs.length > 5 && (
              ...
              >
                See all ({validLikedSongs.length})
              </button>
            )}
          </div>
          ...
          {validLikedSongs.length === 0 ? (
            ...
            <div className="overflow-hidden rounded-2xl border border-white/[0.07] bg-[#101011]">
              <div className="divide-y divide-white/[0.03]">
                {validLikedSongs.slice(0, tab === "songs" ? undefined : 6).map((song, i) => (
                  <SongRow key={song._id || song.id || i} song={song} queue={validLikedSongs} index={i} showIndex />
                ))}
              </div>
            </div>
